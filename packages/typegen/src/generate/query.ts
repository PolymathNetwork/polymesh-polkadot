...
function entrySignature (allDefs: object, registry: Registry, storageEntry: StorageEntryMetadataLatest, imports: TypeImports): [string, string] {
  ...
  const formatIfApplicable = (type: string): string => isUnorthodoxType(type) ? type : formatType(allDefs, type, imports);
  ...
  if (storageEntry.type.isPlain) {
    ...
    setImports(allDefs, imports, [
      ...similarTypes.filter(type => !isUnorthodoxType(type)),
      storageEntry.type.asMap.value.toString()
    ]);

    return [
      `arg: ${similarTypes.map(formatIfApplicable).join(' | ')}`,
    ...
    setImports(allDefs, imports, [
      ...similarTypes1.filter(type => !isUnorthodoxType(type)),
      ...similarTypes2.filter(type => !isUnorthodoxType(type)),
      storageEntry.type.asDoubleMap.value.toString()
    ]);

    const key1Types = similarTypes1.map(formatIfApplicable).join(' | ');
    const key2Types = similarTypes2.map(formatIfApplicable).join(' | ');
...
function generateModule (allDefs: object, registry: Registry, { name, storage }: ModuleMetadataLatest, imports: TypeImports, isStrict: boolean): string[] {
  ...
  return [indent(4)(`${stringLowerFirst(name.toString())}: {`)]
    ...
    .concat(storage.unwrap().items.map((storageEntry): string => {
      ...
      indent(6)(`${stringLowerFirst(storageEntry.name.toString())}: ${entryType}<${entryTypeArgs.join(', ')}>;`);
    }))
    .concat([indent(4)('};')]);
}
...
/** @internal */
export default function generateQuery (dest = 'packages/api/src/types/augment/query.ts', data = staticData, extraTypes: Record<string, Record<string, object>> = {}, isStrict = true): void {
